...
newtype SVGContent = SVGContent { unSVGContent :: LByteString }
...
diaToSVG dia = SVGContent $ Svg.renderBS $ renderDia SVG (SVGOptions (mkWidth 250) Nothing "" [] True) dia
...
calendarPage sid year month = do
  ...
  let endDate = fromGregorian (fromIntegral year) month (gregorianMonthLength (fromIntegral year) month)
...
sessionsToPracticeDays year month sessions =
  let monthLen = gregorianMonthLength (fromIntegral year) month
      lengthAndTempo (Session s e t _) = (fromRational $ toRational $ diffUTCTime e s, fromIntegral t)
      ...
      groupByDate date [] = if date == monthLen + 1 then
                               []
                               else replicate (monthLen + 1 - date) []
   in groupByDate 1 sessions

calendar :: Int -> Int -> [PracticeDay] -> Diagram B
calendar year month days  = let
  startDate = fromGregorian (fromIntegral year) month 1
  ...
  in base
    <> snakeGraph (impureNonNull firstWeek) # translate (fromIntegral startDateDOW ^& (-0.5))
    <> atPoints [p2 (0, (-1) * (0.6 + fromIntegral y)) | y <- [1..(length restWeeks)]] (map snakeGraph (toNullable restWeeks))

splits :: Int -> NonNull [a] -> NonNull [NonNull [a]]
splits n xs = let
  (firstN, post) = splitAt n (toNullable xs)
...
calendarBase monthLen startDateDOW =
  position [(p2 (fromIntegral x, fromIntegral y), calendarCell <> calendarText (show (i + 1))) |
    i <- [0..(monthLen - 1)], let x = (i + startDateDOW) `rem` 7, let y = (-1) * (i + startDateDOW) `quot` 7]

tempoToHue :: Double -> Double
tempoToHue tempo = (hue $ toSRGB blue) + (hue (toSRGB red) - hue (toSRGB blue)) *
  (log tempo - log minTempo) / (log maxTempo - log minTempo)

rgbToColor :: Fractional a => RGB a -> Colour a
...
practiceDayToStops day' = concat $ do
  let len = sum $ map (^._1) day'
  let day = map (over _1 (* (1/len))) day'
  (f, r) <- (,) <$> headMay day <*> tailMay day
  let sessions = scanl (\(_, _, p, _) (l2, t) -> (tempoToColor t, p, p + l2, l2)) (tempoToColor (f^._2), 0, f^._1, f^._1) r
...
snakeGraph days = let
  practiceTimes = impureNonNull $ map ((/(8 * hours)) . sum . map (^._1)) (toNullable days)
  len = length practiceTimes
  top = [p2 (0, head practiceTimes)]
    ++ concat (zipWith (\x y -> [p2 (x + 1/6, y), p2 (x + 1/2, y), p2 (x + 5/6, y)]) [0..] (toNullable practiceTimes))
    ++ [p2 (fromIntegral len, last practiceTimes)]
  bottom = reverse $ map reflectY top
  snake = closeLine $ bspline top <> (fromOffsets [(-2 * (last practiceTimes)) *^ unitY]) <> bspline bottom
  stops = mkStops $ concat $ zipWith
