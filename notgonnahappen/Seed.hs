...
data CalParams = CalParams { practiceTimeM :: Double
...
exampleCalParams = CalParams { practiceTimeM = 1.0
                             , practiceTimeSD = 0.75
                             , startTempo = 55.0
                             , endTempo = 200.0
                             , tempoSD = 10.0
...
practiceDaysToSessions sid year month n pdays =
  concat $ zipWith dayToSessions pdays [1..n]
    where
      dayToSessions pday i = let day = fromGregorian (fromIntegral year) month i
                              ...
                              in zipWith3
                                ...
                                (map (^._2) pday) times (tail (impureNonNull times))

generatePracticeDays :: Int -> Seed -> CalParams -> [PracticeDay]
generatePracticeDays n seed params@CalParams{..} = runST $ do
  ...
  fmap (map (map (over _1 (* 3600)))) $ sequence $ zipWith (generateDay params gen) lengths tempoMins
